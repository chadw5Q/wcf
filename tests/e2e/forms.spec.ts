...
test.describe('Hedge posts cart checkout', () => {
  test('calls checkout API when cart has items', async ({ page }) => {
    let checkoutBody: string | null = null;

    await page.route('**/api/create-checkout-session', async (route) => {
      const postData = route.request().postData();
      checkoutBody = postData;
      ...
      await route.fulfill({
        ...
        body: JSON.stringify({
          sessionId: 'cs_mock',
          url: `${origin}/thank-you?kind=checkout`,
        }),
      });
    });

    await page.goto('/hedge-posts');

    const addBtn = page.locator('.add-to-cart-btn:not([disabled])').first();
    await addBtn.click();

    await page.locator('#cart-button').click();
    await expect(page.locator('#cart-modal')).not.toHaveClass(/hidden/);
    ...
    await expect(page).toHaveURL(/\/thank-you\?kind=checkout/);
    await expect(page.getByRole('heading', { name: /Thank you/i })).toBeVisible();
  });
});
